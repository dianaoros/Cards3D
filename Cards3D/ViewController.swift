...
//  Cards3D
//
//  Created by Diana Oros on 5/9/19.
...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    func renderer(_ renderer: SCNSceneRenderer, nodeFor anchor: ARAnchor) -> SCNNode? {
        ...
        if let imageAnchor = anchor as? ARImageAnchor {
            ...
            planeNode.eulerAngles.x = -.pi / 2 //90 degrees
            
            node.addChildNode(planeNode)
            
            if imageAnchor.referenceImage.name == "queen-card" {
                
                if let cardScene = SCNScene(named: "art.scnassets/eevee.scn") {
            ...
            if imageAnchor.referenceImage.name == "art-card" {
                
                if let cardScene = SCNScene(named: "art.scnassets/oddish.scn") {
